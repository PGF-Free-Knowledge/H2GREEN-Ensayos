...
class Dashboard(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.input_name.setPlaceholderText("Nombre Ensayo")
        ...
        param_layout.addWidget(self.input_name)
    ...
    def start_test(self):
        ...
        filename="ensayos/"+nombre+"_"+time.strftime("%Y%m%d_%H%M%S")+".csv"
    ...
    def export_results(self):
        ...
        if len(self.d_data)<10:
            return

        d=np.array(self.d_data)
        f=np.array(self.f_data)

        # rigidez inicial (regresión primeros puntos)
        try:
            n=min(30,len(d))
            coef=np.polyfit(d[:n],f[:n],1)
            rigidez=coef[0]
            print("Rigidez estimada:",rigidez,"N/mm")
        except:
            pass

        # guardar imagen
        exporter=pg.exporters.ImageExporter(self.plot_fd.plotItem)
        exporter.export("ensayos/curva_FD_"+time.strftime("%Y%m%d_%H%M%S")+".png")

        print("Curva F-D exportada")
    ...
    def update_data(self,t,F,D,P,T):
        ...
        # DETECCION FRACTURA
        if len(self.f_data)>20 and not self.fractura_detectada:

            if self.f_data[-1] < 0.8*max(self.f_data):

                print("Posible fractura detectada")

                self.fractura_detectada=True
