...
class Dashboard(QtWidgets.QMainWindow):
    ...
    def update_data(self,t,F,D,P,T):
        ...
        # ROTURA (CORREGIDA)
        # ===============================
        UMBRAL_ACTIVACION = 0.05
        TIEMPO_MIN = 2.0

        if self.running and not self.rotura_detectada:

            if (self.F_max_registrada > UMBRAL_ACTIVACION) and (t > TIEMPO_MIN):

                if F < self.F_max_registrada * 0.8:
        ...
        # ===============================
        try:
            fmax_user = float(self.input_fmax.text())

            if self.running and F >= fmax_user:

                self.estado.setText("● CORTE POR FUERZA MAX")
                self.estado.setStyleSheet("font-size:18px;color:red")
